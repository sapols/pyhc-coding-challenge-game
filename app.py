...
# Example: teams_data = {'team1': {'current_rung': 1, 'points': 0, 'hints': 2, 'skips': 1, 'completed_tasks': []}}
teams_data = {}
...
def get_tasks(rung):
    ...
    if rung_key in tasks:
        return jsonify(tasks[rung_key]['tasks']), 200
    else:
...
@app.route('/api/skip/<team_id>', methods=['POST'])
def skip_task(team_id):
    # Logic to decrement a team's skip count.
    # TODO: maybe add this to a "skipped_tasks" list?
